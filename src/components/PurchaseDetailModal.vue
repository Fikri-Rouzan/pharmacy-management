...
async function fetchPurchaseDetails(id) {
  ...
  try {
    const { data, error } = await supabase
      .from('purchases')
      .select('*, suppliers(name), profiles(name)')
      .eq('id', id)
      .single();
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-50 transition-opacity">
    <div class="w-full max-w-2xl p-6 bg-white rounded-lg shadow-xl transform transition-all">
      ...
      <div v-else-if="purchaseDetails" class="mt-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mb-6">
          <div>
            <p class="text-gray-500">ID Transaksi</p>
            <p class="font-mono text-gray-800">{{ purchaseDetails.id.substring(0, 8) }}</p>
          </div>
          <div>
            <p class="text-gray-500">Tanggal</p>
            <p class="font-semibold text-gray-800">{{ formatDate(purchaseDetails.created_at) }}</p>
          </div>
          <div>
            <p class="text-gray-500">Supplier</p>
            <p class="font-semibold text-gray-800">{{ purchaseDetails.suppliers.name || 'N/A' }}</p>
          </div>
          <div>
            <p class="text-gray-500">Dicatat oleh</p>
            <p class="font-semibold text-gray-800">{{ purchaseDetails.profiles.name || 'N/A' }}</p>
          </div>
           <div>
            <p class="text-gray-500">Total Transaksi</p>
            <p class="font-bold text-lg text-primary">{{ formatCurrency(purchaseDetails.total_amount) }}</p>
          </div>
        </div>

        <h3 class="font-semibold mb-2">Item Dibeli:</h3>
