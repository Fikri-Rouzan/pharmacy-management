...
async function fetchPurchases() {
  loading.value = true;
  const { data, error } = await supabase.from('purchases').select(`id,created_at,total_amount,suppliers(name),profiles(name)`).order('created_at', { ascending: false });
  if (error) console.error('Error fetching purchases:', error); else purchases.value = data;
  loading.value = false;
}
      ...
      <div>
       ...
       <div class="flex items-center gap-2 w-full md:w-auto flex-wrap">
        ...
        <div class="relative w-full md:w-auto flex-grow">
          ...
          <input v-model="searchQuery" type="text" placeholder="Cari ID, Supplier, Karyawan..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary">
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <tbody class="divide-y divide-gray-200">
              <tr v-if="loading"><td colspan="6" class="p-6 text-center text-gray-500">Memuat data...</td></tr>
